        ...
        <div class="row">
            <div class="col-md-12">
                <div class="card">
                    ...
                    <div class="card-body">
                        <h2 class="text-left font-weight-bold">Gallery Setup</h2>

                        <form id="frm" method="post" class="needs-validation" novalidate="" action="{{ route('admin.cms_settings.gallery.store') }}">
                            {{-- @csrf --}}
                            ...
                            <div class="row mt-3 mb-2">
                                ...
                                <div class="col-md-12 pr-0 text-left">
                                    <label for="Images"
                                        class="col-form-label text-nowrap"><strong>Images</strong></label>
                                </div>
                            </div>
                            ...
                            <div class="tab-content" id="myTabContent">
                                ...
                                <div class="tab-pane fade" id="profile" role="tabpanel" aria-labelledby="gallery-tab">
                                    <div class="row">
                                        @if(isset($gallery))
                                        @php $images = $gallery->images ? json_decode($gallery->images) : []; @endphp
                                        @foreach ($images as $idx => $item)
                                        {{-- @dd($item, explode("/",$item->image)) --}}
                                        <div data-type="image" data-imageid="{{ $item->id ?? null }}" class="col-md-4 col-6 pl-2 pr-2 pt-2 img-container" style="max-width: 200px;" data-id="{{ $item->image ?? '' }}">

                                            <div class="ratio-box text-center" data-type="image-ratio-box" data-toggle="tooltip" data-placement="top" title=""
                                                data-original-title="{{ explode("/",$item->image) ? explode("/",$item->image)[2] : ''}}">
                                                ...
                                                <img data-type="preview" class="btn btn-light ratio-img img-fluid p-2 image border dashed rounded" src="{{ asset($item->image) }}" style="cursor: default;">

                                                <span class="badge badge-pill badge-success p-2 w-50 main-tag"
                                                    style="display:none;">Main</span>

                                                <div class="overlay-btn">
                                                    <button data-imageid="{{ $item->id ?? null }}" data-galleryid="{{ $gallery->id }}"
...
<script>
    $(document).ready(function(){
        ...
        function removeImageFromGallery(){
            ...
            gallery_id  = elem.attr('data-galleryid');
            ...
            if(confirm("Are you sure To remove it?")){
                ...
                $.ajax({
                    url     : `{{ route('admin.cms_settings.gallery.destroy','') }}/${gallery_id}`,
